...
func main() {
	...
	if len(m.Paths) < 1 {
		fatalErr = errors.New("パスがありません。backupツールを使って追加してください")
		return
	}
...
func check(m *backup.Monitor, col *filedb.C) {
	...
	counter, err := m.Now()
	if err != nil {
		log.Panicln("バックアップに失敗しました:", err)
	}
	if counter > 0 {
		log.Printf(" %d個のディレクトリをアーカイブしました\n", counter)
		// ハッシュ値を更新します
		var path path
		col.SelectEach(func(_ int, data []byte) (bool, []byte, bool) {
			if err := json.Unmarshal(data, &path); err != nil {
				log.Println("JSON データの読み込みに失敗しました。"+"次の項目に進みます:", err)
				return true, data, false
			}
			path.Hash, _ = m.Paths[path.Path]
			newdata, err := json.Marshal(&path)
			if err != nil {
				log.Println("JSON データの書き出しに失敗しました。"+"次の項目に進みます:", err)
				return true, data, false
			}
